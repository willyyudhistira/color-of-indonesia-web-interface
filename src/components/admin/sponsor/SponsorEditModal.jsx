...
const SponsorEditModal = ({ isOpen, onClose, item, onSave }) => {
    ...
    useEffect(() => {
        if (item) {
            ...
            setImagePreview(item.logo_url ? `http://localhost:4000${item.logo_url}` : null);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg">
                <h3 className="text-xl font-bold mb-6">Edit Sponsor</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label htmlFor="website_url" className="block text-sm font-medium">Link Website</label>
                        <input type="url" name="website_url" value={formData.website_url} onChange={handleChange} className="mt-1 w-full border rounded-md py-2 px-3"/>
                    </div>
                    <div>
                        <label className="block text-sm font-medium">Ganti Logo</label>
